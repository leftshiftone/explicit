...
class CharacterCaseTokenizerTest {
    ...
    fun testTokenizerWithCustomTokens() {
        val tokenizer = CharacterCaseTokenizer()
        val rulez = ExplicitXmlParser().parse(javaClass.getResourceAsStream("/de/price.xml"))
    ...
    fun testTokenizerWithDatetimeTokens() {
        val tokenizer = CharacterCaseTokenizer()
        val rulez = ExplicitXmlParser().parse(javaClass.getResourceAsStream("/de/datetime.xml"))
        Assertions.assertEquals(listOf("ich", "suche", "von", "9.2.", "bis", "15/16.2", "2018").toString(), tokenizer.tokenize("ich suche von 9.2. bis 15/16.2 2018", rulez.tokens).toString())
    }
